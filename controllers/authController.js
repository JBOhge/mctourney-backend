...
exports.signup = catchAsync(async (req, res, next) => {
  //WE ONLY PASS THE FIELDS WE WANT THE USERS TO BE ABLE TO CONTROL NOT THE WHOLE req.body
  //SO USERS CANNOT SET THEIR "role" FIELD OR OTHER RESTRICTED FIELDS
...
exports.login = catchAsync(async (req, res, next) => {
  ...
  //3) If everything is okay generate and send back JWT
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    //Probably not good practice to test wheater a user exists with a given email
    return next(new AppError('There is no user with that email address', 404));
  }

  // 2) Generate random reset token and update user in DB
...
exports.resetPassword = catchAsync(async (req, res, next) => {
  ...
  // 2) Reset password if token in valid
